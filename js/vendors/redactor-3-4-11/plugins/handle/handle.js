(function($R)
{
    $R.add('plugin', 'handle', {
        ...
        _navigate: function(e) {
			...
			if ((arrows.indexOf(key) !== -1 || key === this.keycodes.ENTER) && this._isShown())
			{
                e.preventDefault();
		...
		{
            var key = e.which;
            var ctrl = e.key === 'Shift' || e.key === 'Control' || e.key === 'Meta' || e.key === 'Alt';
            var arrows = [38, 40];
            ...
            // detect
            if (re.test(this.handleStr) && !this.selection.getTextAfterCaret(1).trim())
            {
		...
		_replace: function(e)
		{
            ...
            var replacement = this.handleTrigger + key + ' ';
